...
const UserChat = () => {
    ...
    const fetchTeachers = ()=>{
        ...
        axiosPrivate.get("/chat/teachers")
        .then((res)=>{
            console.log(res);
                ...
                <div className="flex h-screen antialiased text-gray-800">
                    <div className="flex flex-row h-full w-full overflow-x-hidden">
                            ...
                            <div className="flex flex-col mt-8">
                                ...
                                <div className="flex flex-col space-y-1 mt-4 -mx-2 h-full overflow-y-auto">
                                    {teachers && teachers.map((teacher)=>(
                                        <button key={teacher._id} className="flex flex-row items-center hover:bg-gray-100 rounded-xl p-2" onClick={()=>handleChatClick(teacher._id, teacher)}>
                                            <div className="flex items-center justify-center h-8 w-8 bg-indigo-200 rounded-full">
                                                H
                                            </div>
                                            <div className="ml-2 text-sm font-semibold">{teacher.fullname}</div>
                                        </button>
                                    ))}
                        ...
                        <div className="flex flex-col flex-auto h-5/6 p-6">
                            {clicked ? <ChatUi recipientId={recipientId} recipient={recipient} /> : <div className="flex justify-center items-center font-bold text-2xl flex-shrink-0 rounded-b-2xl bg-gray-300 text-gray-600 h-full p-4">Start Chating with your Teacher</div>}
                        </div>
                    </div>
                </div>
            </div>
