...
const UploadCourse = () => {
  ...
  const fetchCourses = (search) => {
    axiosPrivate
      .get(`/teacher/course?page=${currentPage.current}&search=${search}`)
      .then((res) => {
  ...
  return (
    <>
      <div className="w-screen h-screen overflow-x-hidden bg-dashboard-bg ">
        <TeacherNavbar />
        ...
        <div className="w-full h-auto bg-dashboard-bg flex flex-col gap-8 ">
          ...
          <div className="w-full h-auto flex justify-center px-2 py-2 flex-wrap gap-4 overflow-x-hidden">
            {courses?.length !== 0 ? (
              courses?.map((course) => {
                return (
                  <TeacherCourseCard
            ...
            {searchQuery === "" && (
              <div
                className="h-56 w-64 bg-gray-300 rounded-lg flex justify-center items-center cursor-pointer"
                onClick={openModal}
              >
